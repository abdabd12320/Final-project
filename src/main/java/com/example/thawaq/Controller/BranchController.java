...
public class BranchController {
    ...
    @PutMapping("/open/{branchId}")
    public ResponseEntity openBranch(@AuthenticationPrincipal User user,@PathVariable Integer branchId){
        branchService.openBranch(user.getId(), branchId);
        return ResponseEntity.status(200).body("Branch opened");
    }

    @PutMapping("/close/{branchId}")
    public ResponseEntity closeBranch(@AuthenticationPrincipal User user,@PathVariable Integer branchId){
        branchService.closeBranch(user.getId(), branchId);
